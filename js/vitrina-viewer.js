 ...
 *   viewer.update({ ancho: 120, ledActivo: true });
 */
...
// ─── Constantes de acabados y vidrio ────────────────────────────────────────
...
export class VitrinaViewer {
    ...
    _initPostprocessing() {
        ...
        this._bloomPass = new UnrealBloomPass(new THREE.Vector2(w, h), 0.35, 0.55, 0.80);
    ...
    // Las texturas de madera normal/roughness son compartidas entre todos los materiales
    _getNormalTex() {
    ...
    // ─── Construcción de la vitrina ──────────────────────────────────────────

    /**
     * Limpia el grupo y reconstruye toda la geometría según this.state.
     * • Hace dispose() de geometrías para liberar VRAM.
     * • NO hace dispose() de materiales (protege el caché de shaders).
     */
    _build() {
        const s = this.state;
        this._limpiarGrupo();
        ...
        // 1. Base
        this._addBox(s.ancho, altoBase, s.profundidad, matMadera, 0, altoBase / 2, 0);
        this._addBox(s.ancho - 4, 4, s.profundidad - 4,
            new THREE.MeshStandardMaterial({ color: 0x1a1a1a, roughness: 0.5, metalness: 0.3 }),
            0, 2, 0, false);

        // 2. Paneles laterales, trasero, suelo y techo interior
        const em = 1.8;
        this._addBox(em, altoCuerpo, s.profundidad, matMadera, -s.ancho/2 + em/2, altoBase + altoCuerpo/2, 0);
        this._addBox(em, altoCuerpo, s.profundidad, matMadera,  s.ancho/2 - em/2, altoBase + altoCuerpo/2, 0);
        this._addBox(s.ancho, altoCuerpo, 1.5, matMadera, 0, altoBase + altoCuerpo/2, -s.profundidad/2 + 0.75);
        this._addBox(s.ancho, 1.5, s.profundidad, matMadera, 0, altoBase + altoCuerpo - 0.75, 0);
        this._addBox(s.ancho, 1.5, s.profundidad, matMadera, 0, altoBase + 0.75, 0);

        // 3. Corona
        this._addBox(s.ancho, altoCorona, s.profundidad, matMadera, 0, s.alto - altoCorona/2, 0);
        ...
        [[-1,-1],[1,-1],[-1,1],[1,1]].forEach(([sx, sz]) => {
            this._addBox(gp, altoCuerpo, gp, this._matAluminio,
                sx * (s.ancho/2 - gp/2 - 0.3), yPerfil, sz * (s.profundidad/2 - gp/2 - 0.3));
        });
        ...
        for (let i = 1; i <= nE; i++) {
            ...
            this._addBox(s.ancho - em*2 - 1, 0.8, s.profundidad - 2, matEntrepaño, 0, yE, 0, false);
        ...
        ledTop.position.set(0, s.alto - altoCorona - 0.8, s.profundidad/2 - 3);
        ...
        if (s.ledActivo) {
            ...
            plTop.position.set(0, s.alto - altoCorona - 2, 0);
        ...
        if (s.tipoPuertas === 'abatible') {
            this._construirPuertasAbatibles(s.numPuertas, s.ancho, altoPuerta, yPuerta, zFront, 0.55, matCristal, s.apertura);
        } else {
            this._construirPuertasCorredizas(s.numPuertas, s.ancho, altoPuerta, yPuerta, zFront, 0.55, matCristal, s.apertura);
        }
